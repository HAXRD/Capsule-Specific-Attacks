...
def _read_input(filename_queue):
    ...
    Each record consists a 3x32x32 image with one byte for the label.

    Args:
        filename_queue: A queue of strings with filenames to read from.
    Returns:
        image: a [32, 32, 3] float32 Tensor with the image data.
        label: an int32 Tensor with the labelin the range 0..9
    """
    height = 32
    depth = 3
...
def _distort_resize(image, image_size):
    """Distorts input images for CIFAR10 training.
...
def _batch_features(image, label, batch_size, split, image_size):
    ...
    Batches the images and labels according to the split. Shuffles the data only
...
def inputs(split, data_dir, batch_size):
    ...
    Returns:
        batched_features: A dictinary of the input data features.
    """
    if split == 'train':
