...
training and evaluation ops. Therefore, we define the inference function as
an abstract method that each model should define specifically for itself.
"""
...
class Model(object):
    ...
    def __init__(self, hparams):
        ...
        with tf.device('/cpu:0'):
            self._global_step = tf.get_variable(
                ...
                trainable=False) # init 'global_step'
    ...
    def inference(self, feature):
        ...
        Builds the architecture of the neural net to derive logits from feature.
        The inference graph defined here should involve trainable variables,
        otherwise, the optimizer will raise a ValueError.

        Args:
            feature: Dictionary of batched feature tensors like images and labels.
        Returns:
            An Inferred name tuple for expected outputs of the model like 'logits'
    ...
    def _single_tower(self, tower_ind, feature):
        ...
        gradients based on the loss. Appends all the output values of this tower
        to th repsective lists.

        Args:
            tower_ind: The index number for this tower. Each tower is named as
                        tower_{tower_ind} and resides on /gpu:{tower_ind}.
            feature: Dictionary of batched feature like images and labels.
        Returns:
    ...
    def _average_gradients(self, tower_grads):
        ...
        Args:
            tower_grads: List of gradient lists for each tower. Each gradient list
                         is a list of (gradient, variable) tuple for all variables!!!
        Returns: 
            List of pairs of (gradient, variable) where the gradient has been averaged 
            across all the towers.
        """
        averaged_grads = []
    ...
    def multi_gpu(self, features, num_gpus):
        """Builds the Graph and add the train ops on multiple GPUs.

        Divides the inference and gradient computation on mutiple GPUs.
        Then aggregates the gradients and return the resultant ops.

        Args:
            features: A list of dictionaries of different features of input data.
        ...
        Returns:
            ...
            summaries and a list of inferred outputs of each tower.
        """
        almosts = []
        corrects = []
        tower_grads = []
