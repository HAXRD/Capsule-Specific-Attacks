...
Use only convolutional and fully connected layers for the inference ops.
"""
...
class CNNModel(model.Model):
    """A baseline muti GPU Model without capsule layers.
    ...
    def _add_convs(self, input_tensor, channels):
        ...
        Args:
            ...
            channels: A list of channel sizes for input_tensor and following 
                        convolution layers. Number of channels in input tensor 
                        should be equal to channels[0].
        Returns:
        ...
        for i in range(1, len(channels)):
            with tf.variable_scope('conv{}'.format(i)) as scope:
                kernel = variables.weight_variable(
                    ...
                    stddev=5e-2,
                    verbose=self._hparams.verbose) # (filter_h, filter_w, in_ch, out_ch)
                conv = tf.nn.conv2d(
                    input_tensor,
                    kernel,
                    strides=[1, 1, 1, 1],
    ...
    def inference(self, feature):
        ...
        with tf.variable_scope('fc1') as scope:
            ...
            weights = variables.weight_variable(
                shape=[dim, 1024], stddev=0.1, verbose=self._hparams.verbose)
            biases = variables.bias_variable(
                shape=[1024], verbose=self._hparams.verbose)
