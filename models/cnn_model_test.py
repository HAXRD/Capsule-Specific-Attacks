...
class CNNModelTest(tf.test.TestCase):
    ...
    def testIntegrity(self):
        ...
        with tf.Graph().as_default():
            test_model = cnn_model.CNNModel(self.hparams)
            ...
            _, tower_output = test_model.multi_gpu([features, features], 2)
            trainable_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
            self.assertEqual(len(trainable_vars), 8) # (2 conv + 2 fc) * (1 weights + 1 biases)
